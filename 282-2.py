    ...
    def addOperators(self, num: str, target: int) -> List[str]:
        ...
        def list_all_terms(num, start_idx, term_sequence:List):
            ...
            if num[start_idx] == '0':
                new_term = (0, "0")
                # keep new_term as is
                term_sequence.append(new_term)
                list_all_terms(num, start_idx+1, term_sequence)                
                term_sequence.pop()

                # multiply new term with previous term
                if len(term_sequence)>0:
                    prev_term_operand, prev_term_str = term_sequence[-1]
                    term_sequence[-1] = (prev_term_operand*new_term[0], prev_term_str+"*"+new_term[1])
                    list_all_terms(num, start_idx+1, term_sequence)  

                    term_sequence[-1] = (prev_term_operand, prev_term_str)
            else:
                for i in range(start_idx+1, n+1):
                    ...
                    # keep new term as is
                    list_all_terms(num, i, term_sequence)
                    term_sequence.pop()
