...
class Solution:
    def criticalConnections(self, n: int, connections: List[List[int]]) -> List[List[int]]:
        ...
        disc = [-1] * n # redundant for debugging
        low = [n] * n
                ...
                if v != u and v != parent:
                    ...
                    if disc[v]>=0: # u->v is back edge
                        ...
                        if disc[u] <= low[v]: # v sub stree completed at this time AND v sub tree does not above me
                            if DEBUG:
                                print(f"    disc[{u}]={disc[u]} < low[{v}]={low[v]}")

                            if disc[u] < low[v]: # u is not port of v sub tree
                                if DEBUG:
                                    print(f"        found critical edge: {(u, v)}")
