...
class Solution:
    def deleteMiddle(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        # The problem defines middle as index n//2, so for even-length lists
        # this is the right middle node.

        # Use two pointers moving at different speeds.
        # premid advances one node for every two nodes advanced by pfast.
        # When pfast reaches the end, premid points to the node before the middle node.
        # For an even-sized list, the middle node is the right middle node.        
        premid = None # as if premid pointer to a virtual node before head
        pfast = head
