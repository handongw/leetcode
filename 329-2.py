    ...
    def longestIncreasingPath(self, matrix: List[List[int]]) -> int:
        ...
        def DFS(row, col):
            ...
            if upper_row>=0 and val > matrix[upper_row][col]:
                upper_lip = DFS(upper_row, col) + 1

            right_lip = 0
            right_col = col+1
            if right_col<n and val > matrix[row][right_col]:
                right_lip = DFS(row, right_col) + 1

            down_lip = 0
            down_row = row + 1
            if down_row < m and val > matrix[down_row][col]:
                down_lip = DFS(down_row,col) + 1
            
            left_lip = 0
            left_col = col - 1
            if left_col >= 0 and val > matrix[row][left_col]:
                left_lip = DFS(row,left_col) + 1
   
            lip = max(1, upper_lip, right_lip, down_lip, left_lip)
