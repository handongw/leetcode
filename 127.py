...
class Solution:
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        # --- REUSABLE DIRECTIONAL EXPANSION FUNCTION ---
        def expand_frontier(current_queue, current_state, target_state, total_steps):
            """
            Expands a single layer of the given queue.
            Returns total_steps if an intersection is found, otherwise returns None.
            """
            queuex = list(current_queue)
            current_queue.clear()
            
            for w in queuex:
                if DEBUG:
                    print(f"    State {current_state} expanding: {w} | Total steps so far: {total_steps}")
                ...
                for key in gen_adjacent_map_keys(w):
                    ...
                    for candidate in candidates:
                        ...
                        if st == 0:  # Unvisited node
                            current_queue.append(candidate)
                            state_map[candidate] = current_state
                        elif st == target_state:  # Look look look! Intersection found!
                            if DEBUG:
                                print(f"  Intersection found! Encountered {candidate}")
                            return total_steps
            return None

        # --- REFACTORED BIDIRECTIONAL BFS LOOP (WITH FRONTIER SIZE OPTIMIZATION) ---
        while queue1 and queue2:
            # Always expand the smaller frontier to minimize the search space
            if len(queue1) <= len(queue2):
                result = expand_frontier(queue1, current_state=1, target_state=2, total_steps=queue1_steps + queue2_steps)
                if result: 
                    return result
                queue1_steps += 1            
            else:
                result = expand_frontier(queue2, current_state=2, target_state=1, total_steps=queue1_steps + queue2_steps)
                if result: 
                    return result
                queue2_steps += 1         
