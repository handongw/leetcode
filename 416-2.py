...
class Solution:
    def canPartition(self, nums: List[int]) -> bool:
        ...
        sum_array_size = half_sum+1
        sum_array = [None] * (sum_array_size)
        sum_array[0] = nums[:]

        for k in range(1, len(sum_array)):
            if sum_array[k] is not None:
                continue
            # find a solution for sum = k
            for i in reversed(range(k)):
                num_list = sum_array[i]
                if num_list is not None and num_list:
                    # num_list is sorted
                    if num_list[0] > k-i:
                        k2 = i+num_list[0]  # future result
                        if k2 < sum_array_size and sum_array[k2] is None:
                            sum_array[k2] = num_list[:]
                            del sum_array[k2][0:1]
                            if DEBUG:
                                print(f" preset sum list k={k2}")
                            if k2 == half_sum:
                                return True    
                        continue
                    if num_list[-1] < k-i:
                    ...
                    #     if num_list[idx+1] == k-i:
                    #         idx = i
                    #         break
                    #     elif num_list[idx+1] > k-i:
                    #         break;
                    # if idx >= 0:
                    #       sum_array[k] = num_list[:]
                    #       del sum_array[k][idx:idx+1]
                    #       break  
                    idx = bisect.bisect_left(num_list, k-i)
                    if idx < len(num_list):
                        if num_list[idx] == k-i:
                            # found
                            sum_array[k] = num_list[:]
                            ...
                            k2 = i+num_list[idx]  # future result
                            if k2 < sum_array_size and sum_array[k2] is None:
                                sum_array[k2] = num_list[:]
                                del sum_array[k2][idx:idx+1]
                                if DEBUG:
                                    print(f" preset sum list k={k2}")   
                                if k2 == half_sum:
                                    return True    

        return sum_array[-1] is not None                
