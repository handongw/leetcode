...
class Solution:
    def minWindow(self, s1: str, s2: str) -> str:
        ...
        # state = [ [ None for _ in range(n)] for _ in range(2) ]
        state = [ None for _ in range(n)]
        workingState = [ None for _ in range(n)]
        ...
        for i in range(1, m):
            ...
            for j in range(0, n):
                if j < i:
                    ...
                    workingState[j] = state[j-1]
                    last_p = workingState[j]
                    if DEBUG:
                        print(f"    s2[{i}] == s1[{j}] last_p={last_p}")
                else:
                    if DEBUG:
                        print(f"    s2[{i}] != s1[{j}] state[{i}][{j}] = {last_p}")
                    workingState[j] = last_p
            if DEBUG:
                print(f"state[{i}]={workingState}")
            tmp = state    
            state = workingState              
            workingState = tmp
